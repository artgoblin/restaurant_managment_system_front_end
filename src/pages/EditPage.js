...
const EditPage = ({ 
    ...
    const invoiceAmount = selectedInvoiceDetails.length === 0 ? '' : selectedInvoiceDetails[0]['serving']
    const notes = selectedInvoiceDetails.length === 0 ? '' : selectedInvoiceDetails[0]['food_item'];
    ...
    const handleSave = () => {
        if(newInvoiceAmount !== '') {
            axios.post('http://localhost:8081/Order/Edit', 
            {
                invoiceNumber, 
                newInvoiceAmount,
                newNotes
